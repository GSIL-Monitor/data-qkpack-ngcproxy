...
if [ x"$PID" != x ];then
	echo "[data-qkpack-ngproxy] nginx $PID already is running."
	exit 1;
fi
...
if [ x"$PID" != x ];then
	echo "[data-qkpack-ngproxy] $BIN -c $CONF $PID start success."
else
	echo "[data-qkpack-ngproxy] $BIN -c $CONF start failure."
fi
